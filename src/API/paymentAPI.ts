...
const paymentAPI = {
  ...
  getAllPayments: async (): Promise<AxiosResponse> => {
    return axios.get(`${apiURL}/payment`, config);
  },

  // Get payment by ID
  getPaymentById: async (id: string): Promise<AxiosResponse> => {
    return axios.get(`${apiURL}/payment/${id}`, config);
  },

  // Create a new payment
  createPayment: async (paymentData: PaymentData): Promise<AxiosResponse> => {
    return axios.post(`${apiURL}/payment`, paymentData, config);
  },

  // Update payment by ID
  updatePayment: async (id: string, paymentData: PaymentData): Promise<AxiosResponse> => {
    return axios.put(`${apiURL}/payment/${id}`, paymentData, config);
  },

  // Delete payment by ID
  deletePayment: async (id: string): Promise<AxiosResponse> => {
    return axios.delete(`${apiURL}/payment/${id}`, config);
  },
};
